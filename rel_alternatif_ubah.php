<?php
function get_subkriteria_option($kriteria, $selected = 0){
    global $db;
    $rows = $db->get_results("SELECT kode_subkriteria, nilai, keterangan FROM tb_subkriteria WHERE kode_kriteria='$kriteria' ORDER BY kode_subkriteria");
    foreach($rows as $row){
        if($row->kode_subkriteria==$selected)
            $a.="<option value='$row->kode_subkriteria' selected>$row->keterangan</option>";
        else
            $a.="<option value='$row->kode_subkriteria'>$row->keterangan</option>";
    }
    return $a;
}
...
<div class="form-group">
    ...
    <select class="form-control" name="ID-<?=$row->ID?>"><?=get_subkriteria_option($row->kode_kriteria, $row->kode_subkriteria)?></select>
</div>
<?php endforeach?>
